...
function menuCheckJob()
    varMenuCheckJob = false
    for _, v in pairs(upgradeMenuAccessJobs) do 
        if QBCore.Functions.GetPlayerData().job.name == v then 
            if upgradeMenuAccessDutyCheck == true then 
                if QBCore.Functions.GetPlayerData().job.onduty then 
                    varMenuCheckJob = true
                else
                    varMenuCheckJob = false
                end 
            else
...
function InitiateMenus(isMotorcycle, vehicleHealth, categories, welcomeLabel)
    ...
    -- RECEIPT SYSTEM FIX - CUSTOM COLORS 
    if GetIsVehiclePrimaryColourCustom(GetVehiclePedIsIn(PlayerPedId(), false)) ~= false then 
        hasCustomColour = true 
        customPrimaryColourR, customPrimaryColourG, customPrimaryColourB = GetVehicleCustomPrimaryColour(GetVehiclePedIsIn(PlayerPedId(), false))
        customSecondaryColourR, customSecondaryColourG, customSecondaryColourB = GetVehicleCustomSecondaryColour(GetVehiclePedIsIn(PlayerPedId(), false))
    else
        hasCustomColour = false 
    end
    -- get all veh mods... 
    getVehiclePropertiesReceipt = QBCore.Functions.GetVehicleProperties(GetVehiclePedIsIn(PlayerPedId(), false))
